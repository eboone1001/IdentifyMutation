...
void plotGenes(std::list<annotation> genes, std::string outf, int genomeLength,Mutations mutant, int width = 1920, int height = 1080)
{
	...
	for (std::list<annotation>::iterator it = genes.begin(); it != genes.end(); it++)
	{
		...
		outfile << "\tctx.fillStyle = \"" << hex_color() << "\";\n";
		outfile << "\tctx.fillRect(" << X << ", 0, " << Y << ", 50); \n";
		//names.push_back("\tctx.fillStyle = \"#000000\";\n");
		...
		if (disp) {
			names.push_back("\tctx.font = \"" + std::to_string(fontSize) + "px Verdana\";\n");
			names.push_back("\tctx.fillText(\"" + geneName + "\", " + std::to_string(X + 5) + ", 30); \n");
		}
		/*
		collect relevent information and name it as a protein name, output on first fasta line
		*/
	...
	//add mutations
	for (std::map<std::string, std::list<int>>::iterator it = mutant.getMutations()->begin(); it != mutant.getMutations()->end(); it++)
	{
		std::string color = hex_color();
		for (std::list<int>::iterator itt = it->second.begin(); itt != it->second.end(); itt++)
		{
			int X = int(*itt*scale);
			int Y = 1;
			//int cC = rand() % colLen;
			//outfile << "\tctx.fillStyle = \"" << colors[cC] << "\";\n";
			outfile << "\tctx.fillStyle = \"" << color << "\";\n";
			outfile << "\tctx.fillRect(" << X << ", 0, " << Y << ", 50); \n";
		}
		/*
		TODO output legend
		using color and the name of the current it
		*/
	}
	//add names
	outfile << "\tctx.fillStyle = \"" << "#000000" << "\";\n";
